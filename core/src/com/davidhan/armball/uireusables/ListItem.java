package com.davidhan.armball.uireusables;
...
import com.davidhan.armball.uireusables.scene2dhan.HanTable;
...
public class ListItem extends HanTable {
    ...
    private void build() {
        ...
        titleColor = Colors.get(ColorNames.TEXT_REG);
        if(onClickListener != null){
            ...
            titleColor = Colors.get(ColorNames.TEXT_DISABLED);
        }
        valueLabel = new HanLabel(iApp,value);
        if(actionableValue) {
            valueLabel.setColor(Colors.get(ColorNames.TEXT_REG));
        }else{
            Colors.get(ColorNames.TEXT_DISABLED);
        }
