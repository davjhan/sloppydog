...
public abstract class CompoundPhysicalEntity extends BodyEntity {
    ...
    public void translateBodiesBy(float dx, float dy){
        translateBodiesBy(bodies,dx,dy);
    }
    public void translateBodiesBy(List<Body> bodies,float dx, float dy){
        for(Body body:bodies){
            body.setTransform(body.getPosition().add(dx,dy),body.getAngle());
        }
    }
    public void rotateBodiesBy(Body leader,float degrees){
        rotateBodiesBy(bodies,leader,degrees);
    }
    public void rotateBodiesBy(List<Body> bodies,Body leader,float degrees){
    ...
    public void translateBodiesTo(List<Body> bodies,Body leader, Vector2 destination){
        translateBodiesTo(leader,destination.x-leader.getPosition().x,destination.y-leader.getPosition().y);
    }
    ...
    public void translateBodiesTo(Body leader, float x, float y){
        translateBodiesBy(x-leader.getPosition().x,y-leader.getPosition().y);
    }
